...
void main() {
  testWidgets(
    'missing DATA table shows recovery, retry and the original diagnosis',
    (tester) async {
      ...
      expect(
        find.text('El catálogo de Ítems necesita su tabla real'),
        findsOneWidget,
      );
      ...
      await tester.pumpAndSettle();
      expect(
        find.textContaining('Falta BinarySData/DBItemData.SData'),
        findsOneWidget,
      );
  ...
  testWidgets(
    'locked SPK distinguishes metadata from read access and gates reference validation',
    (tester) async {
      final root = await Directory.systemTemp.createTemp('r28-spk-recovery-');
      addTearDown(() => root.delete(recursive: true));
      final source = await resourceSpkFixture(root, {
        for (var i = 0; i < 3; i++)
          'script/$i.txt': Uint8List.fromList(
            utf8.encode('resource $i test content'),
          ),
      });
      ...
      Widget panel() => MaterialApp(
        home: Scaffold(
          body: ItemsSourceRecovery(
            library: lib,
            error: 'fixture missing table',
            onRetry: () {},
            onRecovery: actions.add,
          ),
        ),
      );
      await tester.pumpWidget(panel());
      expect(
        find.text('4 registros indexados · 0 rutas confirmadas'),
        findsOneWidget,
      );
      expect(
        find.text('Lectura simple: pendiente · fragmentados: pendientes'),
        findsOneWidget,
      );
      expect(
        tester
            .widget<OutlinedButton>(
              find.byKey(const ValueKey('items-confirm-paths')),
            )
            .onPressed,
        isNull,
      );
      ...
      await tester.pump();
      expect(
        find.text('Lectura simple: habilitada · fragmentados: pendientes'),
        findsOneWidget,
      );
