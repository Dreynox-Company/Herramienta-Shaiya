...
void main() {
  ...
  test('loose DATA without Character remains available to the resource editor', () async {
    ...
    expect(catalog.warnings.any((w) => w.contains('biblioteca de recursos')), isTrue);
  ...
  test('SAH/SAF resource library does not need an equipped character', () async {
    final bytes = Uint8List.fromList(utf8.encode('Archive resource without rig.'));
    ...
    final lib = Library('synthetic-archive', false, {path: path}, archive: source);
    addTearDown(lib.dispose);
    final catalog = Catalog(lib);
    await catalog.load((_) {}, requireArchetypes: false);
    expect(catalog.archetypes, isEmpty);
  ...
  test('SPK partial catalog retains all IDs and missing-rig diagnostics', () async {
    ...
    final source = await resourceSpkFixture(root, {
      ...
      'script/b.txt': Uint8List.fromList(utf8.encode('Second readable text.')),
    });
    ...
    expect((await index.read(index.filter('humf_upper', 'Todos').single)).format, 'MLT');
