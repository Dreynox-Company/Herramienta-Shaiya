...
void main() {
  ...
  test(
    'native export rejects fractional, non-finite and overflowing angles',
    () {
      ...
      for (final bad in [
        ...
        expect(
          () => validateNativeWingProfile(original.copyWith(rotX: bad)),
          throwsFormatException,
        );
        expect(
          () => validateNativeWingProfile(original.copyWith(rotY: bad)),
          throwsFormatException,
        );
        expect(
          () => validateNativeWingProfile(original.copyWith(rotZ: bad)),
          throwsFormatException,
        );
      }
  ...
  test(
    'native offsets are finite float32 while translation may be fractional',
    () {
      final original = WingPositionDocument.verifiedResolve(0, 0, 0)!;
      expect(
        () => validateNativeWingProfile(original.copyWith(upDown: 0.125)),
        returnsNormally,
      );
      for (final bad in [double.nan, double.infinity, 1e39]) {
        expect(
          () => validateNativeWingProfile(original.copyWith(upDown: bad)),
          throwsFormatException,
        );
        expect(
          () => validateNativeWingProfile(original.copyWith(frontBack: bad)),
          throwsFormatException,
        );
        expect(
          () => validateNativeWingProfile(original.copyWith(leftRight: bad)),
          throwsFormatException,
        );
      }
      expect(
        () => validateNativeWingProfile(original.copyWith(boneIndex: -1)),
        throwsFormatException,
      );
    },
  );
  test(
    'rejected native pose does not write or mutate the mounted document',
    () async {
      final library = RefusingWingLibrary();
      final catalog = Catalog(library);
      final document = WingPositionDocument.parse(
      ...
      await expectLater(
        catalog.saveWingPosition(
          document.resolve(0, 0, 0)!.copyWith(rotX: 12.5),
        ),
        throwsFormatException,
      );
  ...
  test('late write failure preserves every mounted wing profile', () async {
    ...
    final before = document.encode();
    await expectLater(
      catalog.saveWingPosition(document.resolve(0, 0, 0)!.copyWith(rotX: 12)),
      throwsStateError,
    );
