...
void main() {
  ...
  test(
    'WingPosition blocks duplicate identities and incomplete profile matrix',
    () {
      final doc = ExcelXmlDocument.parse(
        workbook(
          const [
  ...
  test(
    'RenownShop rejects invalid IDs/counts but permits zero optional limits',
    () {
      final doc = ExcelXmlDocument.parse(
        workbook(
          const [
      ...
      final issues = auditExcelXmlSemantics(doc);
      expect(issues.any((issue) => issue.code == 'duplicate-key'), isTrue);
      expect(issues.any((issue) => issue.message.contains('ItemID')), isTrue);
      expect(issues.any((issue) => issue.message.contains('Renown')), isTrue);
    },
  );

  test(
    'time notices validate calendar field ranges when fields are present',
    () {
      final doc = ExcelXmlDocument.parse(
        workbook(
          const [
