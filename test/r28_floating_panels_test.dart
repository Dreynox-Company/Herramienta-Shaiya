...
void main() {
  ...
  for (final side in ['left', 'right']) {
    testWidgets(
      '$side detaches, moves, resizes, maximizes and redocks without losing fields',
      (tester) async {
        ...
        final before = tester.getSize(
          find.byKey(const ValueKey('test-viewport')),
        );
        ...
        await tester.pumpAndSettle();
        expect(
          tester.getSize(find.byKey(const ValueKey('test-viewport'))),
          detached,
        );
        final panel = tester.getRect(find.byKey(ValueKey('floating-$side')));
        await tester.dragFrom(
          panel.topLeft + const Offset(45, 17),
          const Offset(25, 12),
        );
        await tester.pumpAndSettle();
        expect(
        ...
        await tester.tap(find.byKey(ValueKey('maximize-$side')));
        await tester.pumpAndSettle();
        expect(
          tester.getSize(find.byKey(const ValueKey('test-viewport'))),
          detached,
  ...
  testWidgets(
    'dock expansion reserves a useful central area without hiding the other dock',
    (tester) async {
      ...
      await tester.pumpWidget(workspace());
      await tester.drag(
        find.byKey(const ValueKey('resize-left-dock')),
        const Offset(300, 0),
      );
      await tester.pumpAndSettle();
      await tester.drag(
        find.byKey(const ValueKey('resize-right-dock')),
        const Offset(-300, 0),
      );
      await tester.pumpAndSettle();
      expect(
        tester.getSize(find.byKey(const ValueKey('test-viewport'))).width,
        greaterThanOrEqualTo(380),
      );
      expect(
        find.byKey(const ValueKey('right-editor')).hitTestable(),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey('left-editor')).hitTestable(),
        findsOneWidget,
      );
      expect(tester.takeException(), isNull);
      await tester.pumpWidget(const SizedBox());
