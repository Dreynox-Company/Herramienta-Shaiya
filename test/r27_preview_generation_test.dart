...
class PendingPreviewIndex extends ResourceIndex {
  ...
  PendingPreviewIndex()
    : super(Library('preview-fixture', false, {
        'a.ani': 'not-read',
        'b.ani': 'not-read',
        'c.ani': 'not-read',
      }));
  ...
  @override
  Future<ResourceRead> read(ResourceEntry entry, {int limit = 64 * 1024 * 1024}) {
    return pending.putIfAbsent(entry.key, Completer<ResourceRead>.new).future;
  }

  void complete(String key, List<int> bytes) {
    pending[key]!.complete(ResourceRead(
      entries.firstWhere((e) => e.key == key), Uint8List.fromList(bytes), 'ANI',
    ));
  }
}

void main() {
  testWidgets('changing selection removes old bytes while the new resource loads', (tester) async {
    final index = PendingPreviewIndex();
    addTearDown(index.dispose);
    final selected = ValueNotifier<ResourceEntry>(index.entries.first);
    addTearDown(selected.dispose);
    await tester.pumpWidget(MaterialApp(home: Scaffold(body:
      ValueListenableBuilder<ResourceEntry>(valueListenable: selected,
        builder: (_, entry, _) => ResourcePreview(index: index, entry: entry)),
    )));
  ...
  testWidgets('late completion cannot replace a newer selection or rebuild after disposal', (tester) async {
    final index = PendingPreviewIndex();
    addTearDown(index.dispose);
    final selected = ValueNotifier<ResourceEntry>(index.entries.first);
    addTearDown(selected.dispose);
