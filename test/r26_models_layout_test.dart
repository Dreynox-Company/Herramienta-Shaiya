...
Map<String, Uint8List> modelFiles() => {
  itemDataPath: numericItems(),
  itemTextPath: localizedItems(),
...
void main() {
  test(
    'model source families never mix armor, weapons, wings or consumables',
    () {
      final paths = [
        'character/human/humf_upper.mlt',
        'character/elf/elmm_lower.mlt',
        'character/wing/wing.mon',
        'vehicle/vehicle_hu.mon',
        'item/01.itm',
      ];
      expect(ItemModelCatalog.pathsFor(paths, 73), [
        'character/human/humf_upper.mlt',
      ]);
      expect(ItemModelCatalog.pathsFor(paths, 121), [
        'character/wing/wing.mon',
      ]);
  ...
  for (final transport in ['folder', 'SAH/SAF range']) {
    test(
      '$transport uses exact MLT texture pairing; browsing never edits data',
      () async {
        ...
        if (transport == 'folder') {
          ...
          final buffer = BytesBuilder(copy: false),
              index = <String, ArchiveEntry>{};
          for (final entry in entries.entries) {
            index[entry.key] = ArchiveEntry(
              entry.key,
              buffer.length,
              entry.value.length,
              0,
        ...
        final w = ItemWorkspace.fromBytes(
          library,
          entries[itemDataPath]!,
          text: entries[itemTextPath],
        ...
        expect(catalog.choices, hasLength(2));
        expect(
          catalog.choices[0].model.parts.single.$1,
          'character/human/3dc/upper015.3dc',
        );
        expect(
          catalog.choices[0].model.parts.single.$2,
          'character/human/dds/upper016.dds',
        );
        expect(
          catalog.choices[1].model.parts.single.$2,
          'character/human/dds/upper015.dds',
        );
        expect(catalog.choices[0].searchText, contains('016'));
        expect(catalog.choices.every((c) => c.available), isTrue);
        expect(w.dirty, isFalse);
        expect(w.data.exportBytes(), before);
  ...
  ItemModelChoice choice(int id, {bool missing = false}) => ItemModelChoice(
    model: ModelReference(
      'Model $id',
      [('mesh$id.3dc', 'texture$id.dds', 0)],
      sourcePath: source,
      sourceOrdinal: id,
    ),
    name: 'Torso recurso 0$id',
    row: id,
    fields: const {},
    missing: missing ? ['texture$id.dds'] : const [],
  );

  for (final width in [390.0, 800.0, 1366.0]) {
    testWidgets(
      '3D picker preserves cancel/confirm and stale-load safety at $width',
      (tester) async {
            ...
            home: Scaffold(
              body: Builder(
                builder: (ctx) => TextButton(
                  onPressed: () async {
                    opens++;
                    result = await showDialog<ItemModelChoice>(
                      context: ctx,
                      builder: (_) => ItemModelPicker(
                        library: Library('fixture', false, {}),
                        catalog: Future.value(
                          ItemModelCatalog(
                            [choice(15), choice(16), choice(17, missing: true)],
                            [],
                            0,
                          ),
                        ),
                        ...
                        previewBuilder: (context, lib, model, ready) {
                          ...
                          return Center(
                            child: Text(
                              'Native preview test double ${model.sourceOrdinal}',
                            ),
                          );
                        },
                      ),
  ...
  testWidgets(
    'explicit inspector sections move once, remain collapsible and preserve controls',
    (tester) async {
      ...
      addTearDown(tester.view.resetDevicePixelRatio);
      var clicks = 0;
      final docks = StudioDockContent.split(
        Column(
          children: [
            const StudioSection(
              title: 'Alas',
              children: [Text('Selector de alas')],
            ),
            StudioSection(
              title: 'Ajustes de alas',
              children: [
                TextButton(
                  onPressed: () => clicks++,
                  child: const Text('Ajustar XYZ'),
                ),
              ],
            ),
          ],
      ...
      await tester.pumpWidget(
        MaterialApp(
          ...
          home: StudioWorkspace(
            viewport: const Text('3D'),
            left: docks.navigation,
            right: docks.inspector,
            timeline: const SizedBox(),
            actions: const SizedBox(),
            status: const SizedBox(),
            tabs: const ['Alas'],
            icons: const [Icons.view_in_ar],
            selectedTab: 0,
            onTab: (_) {},
            onOpenData: () {},
          ),
      ...
      expect(
        tester.getCenter(find.text('Selector de alas')).dx,
        lessThan(viewport.left),
      );
      expect(
        tester.getCenter(find.text('Ajustar XYZ')).dx,
        greaterThan(viewport.right),
      );
      await tester.tap(find.text('Ajustar XYZ'));
      expect(clicks, 1);
      await tester.tap(find.text('Ajustes de alas'));
      await tester.pumpAndSettle();
