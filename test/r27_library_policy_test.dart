...
void main() {
  ...
  test('a resource-only directory requires an explicit opt-in', () async {
    ...
    await expectLater(
      Library.fromDirectory(root.path, (_) {}),
      throwsFormatException,
    );
    await expectLater(
      Library.fromDirectory(root.path, (_) {}, requireCharacter: true),
      throwsFormatException,
    );
    final resources = await Library.fromDirectory(
      root.path,
      (_) {},
    ...
    expect(
      String.fromCharCodes(await resources.read('script/note.txt')),
      'not a playable character',
    );
  });

  test(
    'nested DATA is normalized in both strict and resource-only mode',
    () async {
      final root = await Directory.systemTemp.createTemp('r27-nested-library-');
      addTearDown(() => root.delete(recursive: true));
      await putText(
        root,
        'client/DATA_Espanol/Character/Human/example.txt',
        'body metadata',
      );
      await putText(
        root,
        'client/DATA_Espanol/Script/example.txt',
        'script metadata',
      );
      for (final strict in [true, false]) {
        final lib = await Library.fromDirectory(
          root.path,
          (_) {},
          requireCharacter: strict,
        );
        addTearDown(lib.dispose);
        expect(lib.files.keys.toSet(), {
          'character/human/example.txt',
          'script/example.txt',
        });
        expect(
          String.fromCharCodes(await lib.read('script/example.txt')),
          'script metadata',
        );
      }
    },
  ...
  test(
    'resource mode does not silently choose one of two nested DATA roots',
    () async {
      final root = await Directory.systemTemp.createTemp(
        'r27-ambiguous-library-',
      );
      addTearDown(() => root.delete(recursive: true));
  ...
  test('resource mode does not weaken canonical relative path validation', () {
    ...
    expect(
      canon(r'Character\Human\EXAMPLE.TXT'),
      'character/human/example.txt',
    );
  });
}
