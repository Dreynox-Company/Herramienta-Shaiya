...
class ItemFieldProfile {
  static FieldMeaning meaning(ItemEntry item, String field) {
    ...
    if ({'itemtype', 'itemtypeid', 'type', 'typeid'}.contains(n)) {
      return FieldMeaning(
        n.contains('id') ? 'ID dentro del tipo' : 'Tipo nativo',
        'Identidad',
        'Identidad estable Type:TypeId. Reasignarla requiere migrar todas sus referencias; no se cambia como un stat.',
      );
    }
    if ({'itemname', 'name', 'text', 'description'}.contains(n)) {
      return FieldMeaning(
        ...
        'Texto de la tabla localizada seleccionada. Se conserva la codificación original y no se traduce automáticamente.',
      );
    }
    if (n == 'icon' || n == 'image') {
      return FieldMeaning(
        n == 'icon' ? 'Icono del inventario' : 'Índice visual / Image',
        'Aspecto',
        n == 'icon'
    ...
    if (n == 'consthp' || n == 'constsp' || n == 'constmp') {
      ...
      return FieldMeaning(
        '${item.type == 25 ? 'Recuperación / efecto' : 'Valor constante'} $stat',
    ...
    if (n.startsWith('const') && stats.containsKey(n.substring(5))) {
      return FieldMeaning(
        'Bonificación · ${stats[n.substring(5)]}',
        'Estadísticas',
    ...
    if (stats.containsKey(n)) {
      return FieldMeaning(
        'Requisito · ${stats[n]}',
        'Requisitos',
        'Campo original $field; distinto de Const${field[0].toUpperCase()}${field.substring(1)}. Se conserva incluso cuando el tipo no lo utiliza.',
      );
    }
    if (RegExp(r'^effect[1-4]$').hasMatch(n)) {
      final group = category == 'Armas'
          ? 'Daño y efectos'
          : category == 'Armaduras' || category == 'Escudos'
          ? 'Defensa y efectos'
          : category == 'Lapisias' || category == 'Lapis y materiales'
          ? 'Mejora y efectos'
          : 'Uso y efectos';
      return FieldMeaning(
        'Parámetro de efecto ${n.substring(6)}',
        group,
    ...
    if ({
      ...
      const labels = {
        ...
        'itemskill': 'Habilidad del ítem',
        'usecontype': 'Tipo de condición de uso',
        'useconvar': 'Valor de condición de uso',
        'casttime': 'Tiempo de uso / lanzamiento',
        'duration': 'Duración',
        'extduration': 'Duración extendida',
      };
      return FieldMeaning(
        labels[n]!,
        category == 'Lapisias' ? 'Mejora y efectos' : 'Uso y efectos',
    ...
    if ({'range', 'attacktime', 'attrib', 'quality', 'slot'}.contains(n)) {
      const labels = {
        'range': 'Alcance',
        'attacktime': 'Intervalo de ataque',
        'attrib': 'Atributo / elemento',
        'quality': 'Calidad / durabilidad',
        'slot': 'Ranuras',
      };
      return FieldMeaning(
        labels[n]!,
        category == 'Armas' ? 'Daño y efectos' : 'Equipamiento',
    ...
    if ({
      ...
      return FieldMeaning(
        n == 'level'
            ...
            : base.label,
        'Requisitos',
        'Restricción original $field. Cambiarla aquí no modifica automáticamente la validación del servidor.',
      );
    }
    if ({
      'buy',
      ...
      const labels = {
        ...
        'count': 'Cantidad / apilado',
        'grade': 'Grupo / Grade',
        'drop': 'Parámetro de botín',
        'server': 'Servidor',
        'buymethod': 'Método de compra',
    ...
    if (n == 'mesh' ||
        n == 'texture' ||
        n.contains('meshindex') ||
        n.contains('textureindex') ||
        n.startsWith('parts[')) {
