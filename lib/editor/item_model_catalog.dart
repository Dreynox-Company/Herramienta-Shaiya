...
class ItemModelChoice {
  ...
  String get searchText => foldedSearch(
    '$name $source Image $ordinal '
...
class ItemModelCatalog {
  ...
  static bool supports(int type) =>
      pathsFor(const [], type).isNotEmpty ||
  ...
  static List<String> pathsFor(Iterable<String> paths, int type) {
    ...
    if (prefix != null) {
      return paths
          .where((p) => p.startsWith(prefix!) && p.endsWith('.mon'))
          .toList()
        ..sort();
    }
    final slot = slots.contains(0)
        ? 'helmet'
        : slots.contains(1)
        ? 'upper'
        : slots.contains(2)
    ...
    if (slot == null) return [];
    return paths
        .where((p) => p.startsWith('character/') && p.endsWith('_$slot.mlt'))
        .toList()
      ..sort();
  }

  static Future<ItemModelCatalog> load(
    ...
    final paths = document == null
        ? pathsFor(workspace.preview.files.keys, item!.type)
        : [document.path];
      ...
      try {
        ...
        for (var row = 0; row < opened.rows.length; row++) {
          ...
          for (final model in models) {
            ...
            final resource = model.parts
                .map((p) => '${baseName(p.$1)} + ${baseName(p.$2)}')
                .join(' · ');
            out.add(
              ItemModelChoice(
                model: model,
                name:
                    '${labels.isEmpty ? 'Recurso sin nombre de ítem' : labels.join(' / ')} · $resource',
                row: row,
                missing: missing,
                fields: {
                  for (final f in opened.fields(row))
                    f.spec.name: opened.read(f),
                },
              ),
            );
          }
    ...
    if (workspace.sourceChanged || workspace.revision != revision) {
      throw StateError(
        'La sesión cambió mientras se cargaban los modelos. Vuelve a abrir el selector.',
      );
    }
    return ItemModelCatalog(
