...
class ItemEntry {
  ...
  ItemEntry(this.row,this.type,this.id,this.textRow,this.values,this.name,
      this.description) : category = itemFamilyLabel(type),
      searchText = foldedSearch('$name $description $type:$id ${itemFamilyLabel(type)}');
  String get key => '$type:$id';
  bool get named => name.trim().isNotEmpty &&
      !RegExp(r'^[?\s]+$').hasMatch(name) && !name.contains('\uFFFD');
...
String itemFamilyLabel(int type) {
  ...
  if (equipmentSlotsForItemType(type).any((s)=>s>=0 && s<=4)) return 'Armaduras';
  if ({22,23,24,39,40}.contains(f)) return 'Accesorios';
  if (type==94) return 'Lapisias';
  if ({30,95,98}.contains(type)) return 'Lapis y materiales';
  if ({27,28,29,99,128,129}.contains(type)) return 'Misiones / materiales';
  if (f==25 || {100,101,102,103,130,131}.contains(type)) return 'Consumibles / especiales';
...
class ItemQuery {
  ...
  factory ItemQuery.parse(String input, Iterable<String> fields) {
    if (input.length > 4096) throw const FormatException('Consulta demasiado larga.');
    ...
    for (final m in RegExp(r'"([^"]*)"|(\S+)').allMatches(input)) {
      final token = m[1] ?? m[2]!;
      final cmp = RegExp(r'^([A-Za-z][A-Za-z0-9_]*)(>=|<=|!=|=|>|<)(-?\d+)$').firstMatch(token);
      if (cmp != null) {
        final field = cmp[1]!.toLowerCase(), op=cmp[2]!, value=BigInt.parse(cmp[3]!);
        if (!known.contains(field)) throw FormatException('Campo desconocido: $field');
        tests.add((item) {
          final actual = BigInt.tryParse(item.values[field] ?? '');
          if (actual==null) return false;
          final n=actual.compareTo(value);
          return switch(op) { '>='=>n>=0, '<='=>n<=0, '!='=>n!=0, '>'=>n>0, '<'=>n<0, _=>n==0 };
        });
      } else {
...
class ItemWorkspace {
  ...
  ItemWorkspace(this.data,this.text) : documents={data.path:data, if(text!=null)text.path:text} {
    if (!data.complete || (text != null && !text!.complete)) {
      throw const FormatException('El editor de ítems requiere tablas completas.');
    }
    rebuild();
  }
  static Future<ItemWorkspace> load(Library library,{String? textPath}) async {
    const path='binarysdata/dbitemdata.sdata';
    if (!library.files.containsKey(path)) throw const FormatException(
      'Falta BinarySData/DBItemData.SData. No se sustituye por otra familia de tablas.');
    final choices=ClientLocale.tableCandidates(library.files.keys,'dbitemtext',beside:path);
    final chosen=textPath ?? choices.firstOrNull;
    if (chosen!=null && !choices.contains(chosen)) throw const FormatException('Texto de otra familia.');
    return compute(parse, <String,Object?>{
      'data':await library.read(path), 'path':path,
      'text':chosen==null?null:await library.read(chosen), 'textPath':chosen,
    });
  }
  static ItemWorkspace parse(Map<String,Object?> input) => ItemWorkspace(
    ...
    input['text']==null?null:EditorReader.open(input['text'] as Uint8List,
      input['textPath'] as String,encoding:ClientLocale.encodingForPath(input['textPath'] as String)),
  );
  static Map<String,String> values(EditDocument d,int row) => {
    for(final f in d.fields(row)) if(f.spec.type!='opaque')f.spec.name.toLowerCase():d.read(f),
  };
  static String identity(Map<String,String> v) {
    ...
    if(type==null || id==null || type<BigInt.one || type>BigInt.from(255) ||
        id<BigInt.one || id>BigInt.from(65535)) throw const FormatException('Identidad de ítem inválida.');
    return '$type:$id';
  }
  void rebuild() {
    final names=<String,(int,Map<String,String>)>{};
    if(text!=null) for(var row=0;row<text!.rows.length;row++) {
      final v=values(text!,row), key=identity(v);
      if(names.containsKey(key)) throw FormatException('Texto duplicado: $key');
      names[key]=(row,v);
    }
    ...
    for(var row=0;row<data.rows.length;row++) {
      ...
      next.add(ItemEntry(row,int.parse(ids[0]),int.parse(ids[1]),n?.$1,v,
        n?.$2['itemname'] ?? v['itemname'] ?? '', n?.$2['text'] ?? v['text'] ?? ''));
    }
    entries=next; byKey={for(final e in next)e.key:e};
  }
  bool get dirty => documents.values.any((d)=>d.dirty);
  bool get canUndo => _undo.isNotEmpty;
  bool get canRedo => _redo.isNotEmpty;
  List<String> get fieldNames => data.rows.isEmpty ? [] : data.fields(0).map((f)=>f.spec.name).toList();
  List<String> get warnings => [for(final d in documents.values)...d.warnings,
    if(text==null)'Sin DBItemText de esta familia: las propiedades numéricas siguen disponibles.'];
  ...
  void apply(Map<EditDocument,List<(int,FieldSpan,String)>> changes) {
    for(final pair in changes.entries) {
      if(documents[pair.key.path]!=pair.key) throw const FormatException('Documento ajeno a la sesión.');
      for(final (row,f,value) in pair.value) {
        if({'itemtype','itemtypeid','type','typeid'}.contains(f.spec.name.toLowerCase())) {
          throw const FormatException('La identidad requiere migrar sus referencias; no es una propiedad.');
        }
        if(!pair.key.fields(row).any((x)=>x.start==f.start && x.spec.name==f.spec.name)) {
    ...
    } catch (_) { for(final d in applied.reversed)d.undo(); rethrow; }
    if(applied.isEmpty)return;
    _undo.add(applied);_redo.clear();revision++;rebuild();
  }
  void undo() { if(!canUndo)return;final batch=_undo.removeLast();for(final d in batch.reversed)d.undo();_redo.add(batch);revision++;rebuild(); }
  void redo() { if(!canRedo)return;final batch=_redo.removeLast();for(final d in batch)d.redo();_undo.add(batch);revision++;rebuild(); }
  Future<CatalogDocument> catalog(Library lib,String path) async {
    ...
    final d=CatalogDocument.open(await lib.read(path,limit:64*1024*1024),path,GameTextEncoding.automatic);
    documents[path]=d;return d;
  }
  ...
  Future<Directory> export(Library lib,Directory destination) async {
    ...
    if(await FileSystemEntity.type(destination.path,followLinks:false)!=FileSystemEntityType.notFound) {
      throw const FileSystemException('El destino ya existe; elige una carpeta nueva.');
    }
    final outputs=<String,Uint8List>{};
    for(final d in documents.values.where((d)=>d==data || d==text || d.dirty)) {
      final bytes=d.exportBytes();
      final check=d is CatalogDocument?CatalogDocument.open(bytes,d.path,d.codec.encoding)
          :EditorReader.open(bytes,d.path,encoding:d.codec.encoding);
      if(!check.complete || check.rows.length!=d.rows.length)throw FormatException('Relectura fallida: ${d.path}');
      for(final change in d.changes) {
        final f=check.fields(change.row).where((f)=>f.spec.name==change.field.spec.name).single;
        if(check.read(f)!=d.read(change.field))throw FormatException('Cambio perdido: ${d.path} / ${f.spec.name}');
      }
      outputs[d.path]=bytes;
    }
    Future<void> conflicts() async {
      for(final path in outputs.keys) {
        if(FileSave.hash(await lib.read(path))!=documents[path]!.sha)throw FormatException('DATA cambió: $path. No se publicó el lote.');
      }
    }
    await conflicts();await destination.parent.create(recursive:true);
    ...
    try {
      for(final pair in outputs.entries) {
        ...
        await f.parent.create(recursive:true);await f.writeAsBytes(pair.value,flush:true);
        if(FileSave.hash(await f.readAsBytes())!=FileSave.hash(pair.value))throw const FileSystemException('Fallo de integridad.');
      }
      final manifest={
        'schema':1,'kind':'shaiya-studio-item-workspace','iconProfile':ItemIconLayout.profile,
        'sourceDataModified':false,'serverDatabaseModified':false,'gameExecutableModified':false,
        'textSource':text?.path,'warnings':warnings,
        'files':[for(final pair in outputs.entries){'path':pair.key,'beforeSha256':documents[pair.key]!.sha,
          'afterSha256':FileSave.hash(pair.value),'bytes':pair.value.length}],
        'changes':[for(final d in documents.values)for(final c in d.changes){'path':d.path,
          'row':c.row,'field':c.field.spec.name,'before':c.beforeText,'after':d.read(c.field)}],
      };
      await File('${stage.path}/manifest.json').writeAsString(const JsonEncoder.withIndent('  ').convert(manifest),flush:true);
      ...
      await conflicts();
      if(await destination.exists())throw const FileSystemException('Conflicto de destino.');
      return await stage.rename(destination.path);
    } finally {if(await stage.exists())await stage.delete(recursive:true);}
  }
}
...
class _ItemView extends Library {
  ...
  _ItemView(this.workspace,this.source):super(source.location,source.saf,source.files);
  @override Future<Uint8List> read(String path,{int limit=64*1024*1024}) async {
    final d=workspace.documents[canon(path)];
    if(d==null || !d.dirty)return source.read(path,limit:limit);
    final bytes=d.exportBytes();
    if(bytes.length>limit)throw const FormatException('Recurso fuera de límite.');
    return bytes;
  }
