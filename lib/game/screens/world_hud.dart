...
class WorldHud extends StatelessWidget {
  ...
  Widget _playerHud()=>Container(
    ...
    child:Row(children:[
      Container(
        ...
        child:const Icon(Icons.local_fire_department,color:Color(0xffffad3b),size:34),
      ),
      ...
      Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
        Row(children:[
          ...
          Expanded(child:Text(characterName,style:const TextStyle(color:Color(0xffffe742),fontSize:13))),
        ]),
  ...
  Widget _topHotbar()=>Row(children:List.generate(10,(i)=>Container(
    width:39,height:39,
    ...
    child:Stack(children:[
      Center(child:Icon(
        i<2?(i==0?Icons.auto_fix_high:Icons.healing):Icons.circle_outlined,
        color:i<2?const Color(0xffffde82):const Color(0xff8b8374),
        size:23,
      )),
      Positioned(
        top:1,left:2,
        child:Text(((i+1)%10).toString(),style:const TextStyle(fontSize:8,color:Colors.white70)),
      ),
    ]),
  )));
  ...
  Widget _minimap()=>Container(
    ...
    child:Column(children:[
      Expanded(child:CustomPaint(painter:_MiniMapPainter(scene),child:const SizedBox.expand())),
      Row(children:[
        const Text('+  −',style:TextStyle(color:Colors.white70,fontSize:13)),
        const Spacer(),
        Text(
          scene.originX.round().toString()+' '+scene.originZ.round().toString(),
  ...
  Widget _chat()=>Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
    Container(
      width:75,height:31,alignment:Alignment.center,
      decoration:BoxDecoration(
  ...
  Widget _bottomHud()=>Column(children:[
    Container(
      height:13,
      decoration:BoxDecoration(
        color:const Color(0xaa2b241a),
        border:Border.all(color:const Color(0xff8e7c5d)),
      ),
      child:Stack(children:[
        const Center(child:Text('0,0%',style:TextStyle(fontSize:9,color:Colors.white70))),
        FractionallySizedBox(widthFactor:.03,child:Container(color:const Color(0xffd6ad65))),
      ]),
    ),
    Expanded(child:Container(
      color:const Color(0xaa090909),
      child:Align(
        alignment:Alignment.centerRight,
        child:Row(mainAxisSize:MainAxisSize.min,children:[
          for(final icon in [
            Icons.menu_book,Icons.inventory_2,Icons.backpack,Icons.description,
            Icons.pan_tool,Icons.emoji_events,Icons.chat,Icons.sports_martial_arts,
            Icons.settings,Icons.card_giftcard
          ])
            Container(
              width:28,height:28,
              ...
              decoration:BoxDecoration(
                ...
                border:Border.all(color:const Color(0xffc2985c)),
              ),
              child:Icon(icon,size:16,color:const Color(0xffffdfa2)),
            ),
        ]),
      ),
    )),
    ...
    return Container(
      decoration:BoxDecoration(
        ...
        gradient:const LinearGradient(
          colors:[Color(0xffa87a49),Color(0xff684527)],
      ...
      child:Column(children:[
        Container(
          height:34,color:const Color(0xff43251a),
          padding:const EdgeInsets.symmetric(horizontal:10),
          child:Row(children:[
            const Icon(Icons.priority_high,color:Color(0xffffff42),size:17),
            const SizedBox(width:5),
            Expanded(child:Text(
              title,
              maxLines:1,overflow:TextOverflow.ellipsis,
              style:const TextStyle(color:Color(0xffffdc63),fontSize:11,fontWeight:FontWeight.bold),
            )),
          ]),
        ),
        Expanded(child:Padding(
          ...
          child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
            Expanded(child:SingleChildScrollView(
              child:Text(body,style:const TextStyle(color:Color(0xff2d190f),fontSize:10,height:1.45)),
            )),
            const Divider(color:Color(0xff5d3c24)),
            const Text('Objeto de recompensa',style:TextStyle(color:Color(0xff2d190f),fontSize:10)),
            const SizedBox(height:8),
            Container(
              width:34,height:34,
              decoration:BoxDecoration(
                color:const Color(0xffd9cfb6),
                border:Border.all(color:const Color(0xff47311e)),
              ),
        ...
        Padding(
          ...
          child:Row(mainAxisAlignment:MainAxisAlignment.center,children:[
            shaiyaRedButton('Aceptar',onAcceptQuest,width:64,height:29,fontSize:10),
            const SizedBox(width:26),
            shaiyaRedButton('Cancelar',onCancelQuest,width:64,height:29,fontSize:10),
          ]),
        ),
      ]),
    );
...
class _MiniMapPainter extends CustomPainter {
  ...
  @override void paint(Canvas canvas,Size size){
    canvas.drawRect(Offset.zero&size,Paint()..color=const Color(0xff35452b));
    canvas.drawOval(
      Rect.fromCenter(
        center:Offset(size.width*.67,size.height*.27),
        width:size.width*.44,height:size.height*.28,
      ),
      Paint()..color=const Color(0xff476d79),
    );

    final road=Paint()
      ..color=const Color(0xff88744a)
      ..strokeWidth=8
      ..style=PaintingStyle.stroke;
    final path=Path()
      ..moveTo(size.width*.05,size.height*.86)
      ..cubicTo(size.width*.34,size.height*.60,size.width*.46,size.height*.70,size.width*.9,size.height*.14);
