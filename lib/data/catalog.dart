...
String animationLabel(String source) {
  final x=baseName(source).toLowerCase();var label='Animación';
  ...
  final number=RegExp(r'_(\d+)').firstMatch(x)?.group(1)??'';return '$label${number.isEmpty?'':' · $number'}';
}
String setIdentity(String texture) {
  var x=baseName(texture).toLowerCase().replaceFirst(RegExp(r'\.[^.]+$'),'');
  x=x.replaceFirst(RegExp(r'^(hum[fwm]*|huw[fwm]*|elm[mr]*|elw[mr]*|vim[mr]*|viw[mr]*|dem[fr]*|dew[fr]*|pd[bw][mw]f)_'),'');
...
String displaySet(String id) {
  ...
  for(final e in translations.entries){x=x.replaceAll(e.key,e.value);}return 'Conjunto ${x.replaceAll('_',' ')}';
}
class PartRecord {
  final Slot slot;
...
class Catalog {
  ...
  Future<void> load(void Function(String) progress) async {
    ...
    for(final p in paths.where((p)=>p.startsWith('item/')&&p.endsWith('.itm')&&!p.contains('.bak.'))){
      try{for(final w in readItm(await library.read(p),p)){final key='${w.mesh.toLowerCase()}|${w.texture.toLowerCase()}';if(!w.mesh.toLowerCase().startsWith('null.')&&weaponIds.add(key))weapons.add(w);}}catch(e){warnings.add(e.toString());}
    }
...
class Appearance {
  ...
  factory Appearance.forSet(Archetype a,String key,{Appearance? previous}) {
    ...
    final slots=<Slot,PartRecord?>{for(final s in Slot.values)s:null};slots.addAll(set);
    for(final s in [Slot.face,Slot.hair]){slots[s]=previous?.archetype.id==a.id&&previous?.archetype.race==a.race?previous!.selected[s]:((a.parts[s]??[]).isEmpty?null:a.parts[s]!.first);}
  ...
  Appearance withPart(Slot slot,PartRecord? part) {
    ...
    if(slot==Slot.upper&&part!=null){
      ...
      if(set!=null&&!set.containsKey(Slot.lower))return Appearance.forSet(archetype,part.key,previous:this);
    }
    if(slot==Slot.lower&&fullCostume&&part!=null)throw const FormatException('El atuendo integral ya incluye las piernas. Cambia primero el torso por una armadura modular.');
