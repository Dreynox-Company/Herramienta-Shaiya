...
List<String> groundMotionCandidates(
...
class LocomotionTransitions {
  ...
  GroundMotion? update({
    ...
    final next = x != 0 || z != 0
        ? (running ? GroundMotion.run : GroundMotion.walk)
        : GroundMotion.idle;
    final changed = next != _requested;
    _requested = next;
    if (blocked) {
      _wasBlocked = true;
      return null;
    }
    final apply = changed || _wasBlocked || _invalidated;
    _wasBlocked = false;
    _invalidated = false;
