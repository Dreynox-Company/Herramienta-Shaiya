...
class VaniData {
  ...
  static VaniData parse(Uint8List bytes, String source) {
    ...
    final meshCount = r.count(10000),
        ...
        unknown1 = r.i32();
    if (!radius.isFinite || radius < 0) r.fail('Radio VAni inválido.');
    if (meshCount == 0 || frameCount == 0) r.fail('VAni sin mallas o frames.');
    final meshes = <VaniMeshData>[];
    for (var meshIndex = 0; meshIndex < meshCount; meshIndex++) {
      final texture = r.str();
      if (!RegExp(r'\.(dds|tga|bmp|png), caseSensitive: false)
          .hasMatch(name),
    )) {
...
, caseSensitive: false)
          .hasMatch(texture)) {
        r.fail('Textura VAni desconocida.');
      }
      ...
      if (vertexCount == 0 || total > 50000000) {
        r.fail('VAni excede el límite de vértices animados: $total.');
      }
      ...
      final uv = List.generate(
        frameCount,
        (_) => Float32List(vertexCount * 2),
      );
...
SmodData readSmodData(Uint8List bytes, String source) {
  ...
  final center = r.vec(),
      ...
      viewLower = r.vec(),
      viewUpper = r.vec();
  if (!radius.isFinite || radius < 0) r.fail('Radio SMOD inválido.');
...
class DgData {
  ...
  static DgData parse(Uint8List bytes, String source) {
    ...
    if (textureCount == 0) r.fail('DG sin texturas.');
...
class SvmapMobSpawn {
  final int id, count;
  const SvmapMobSpawn(this.id, this.count);
}

class SvmapMobArea {
  final v.Vector3 lower, upper;
  final List<SvmapMobSpawn> mobs;
  const SvmapMobArea(this.lower, this.upper, this.mobs);
}
...
class SvmapSpawnArea {
  final int faction;
  final v.Vector3 lower, upper;
  const SvmapSpawnArea(this.faction, this.lower, this.upper);
}

class SvmapNamedArea {
  final v.Vector3 lower, upper;
  final int name1, name2;
  const SvmapNamedArea(this.lower, this.upper, this.name1, this.name2);
}
...
class SvmapData {
  ...
  static SvmapData parse(Uint8List bytes, String source) {
    ...
    if (cellSize <= 0 || cellSize > mapSize) {
      r.fail('Tamaño de celda SVMAP inválido: $cellSize.');
    }
    ...
    for (var i = 0; i < spawnCount; i++) {
      ...
      final faction = r.i32();
      r.i32();
      spawns.add(SvmapSpawnArea(faction, r.vec(), r.vec()));
    }
    final named = <SvmapNamedArea>[];
    final namedCount = r.count(100000);
    for (var i = 0; i < namedCount; i++) {
      named.add(SvmapNamedArea(r.vec(), r.vec(), r.i32(), r.i32()));
    }
    r.end();
    return SvmapData(
      mapSize,
