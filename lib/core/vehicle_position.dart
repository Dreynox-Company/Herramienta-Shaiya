...
const Map<int, ({int idle, int moving, String label})> riderAnimationProfiles =
    {
...
class VehiclePositionDocument {
  ...
  factory VehiclePositionDocument.empty({String path = canonicalPath}) =>
  ...
  Uint8List encode() {
    final out = StringBuffer()
      ...
      ..writeln(
        '; Missing/disabled sections preserve the native game.exe result.',
      )
      ..writeln(
        '; Position=millimetres, rotation=millidegrees, scale=permille.',
      )
  ...
  static int _integer(Map<String, String> values, String key, int fallback) {
    ...
    if (value == null || value < -0x7fffffff || value > 0x7fffffff) {
      throw FormatException(
        'VehiclePosition.ini: $key no es un entero válido.',
      );
    }
    return value;
  }
