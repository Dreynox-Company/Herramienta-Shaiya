...
/// Reglas del laboratorio, no las fórmulas del servidor de Shaiya.
class Combat {
  ...
  bool attack(double distance, {double? duration}) {
    ...
    _pending.add(_Hit(_clock + time * .45, true));
    onEvent?.call('player', 'attack');
    return true;
  }
  ...
  void step(double dt, double distance) {
    ...
    if (counterattack && distance <= range && _clock >= _nextEnemy) {
      _nextEnemy = _clock + 1.8;
      _pending.add(_Hit(_clock + .55, false));
      onEvent?.call('enemy', 'attack');
    }
    for (final hit in List<_Hit>.from(_pending)) {
      if (hit.at > _clock) continue;
      _pending.remove(hit);
      if (distance > range || !alive) continue;
      if (hit.player) {
        enemyHealth = math.max(0, enemyHealth - damage);
        record('Impacto: ${damage.round()} de daño.');
        onEvent?.call('enemy', enemyHealth == 0 ? 'death' : 'hit');
      } else {
        playerHealth = math.max(0, playerHealth - enemyDamage);
        record('Recibido: ${enemyDamage.round()} de daño.');
        onEvent?.call('player', playerHealth == 0 ? 'death' : 'hit');
      }
      if (!alive) {
        active = false;
        automatic = false;
        ...
        record(
          enemyHealth == 0
              ? 'Prueba terminada: criatura derrotada.'
              : 'Prueba terminada: personaje derrotado.',
        );
      }
    }
  }
