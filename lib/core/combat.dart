...
class Combat {
  ...
  bool attack(double distance,{double duration=1.0}) {
    ...
    active=true;_nextPlayer=_clock+math.max(cooldown,duration);
    _pending.add(_Hit(_clock+duration*.45,true));onEvent?.call('player','attack');return true;
  }
  void step(double dt,double distance) {
    ...
    for(final hit in List<_Hit>.from(_pending)) {
      if(hit.at>_clock)continue;_pending.remove(hit);
