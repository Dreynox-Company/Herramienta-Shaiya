...
class StudioScene extends ChangeNotifier {
  ...
  RenderPart? weapon, secondWeapon;
  ...
  Future<void> setup(t.ThreeJS three) async {
    ...
    final gridGeometry = t.BufferGeometry()
    ...
    three.scene.add(
      t.LineSegments(
        gridGeometry,
        t.LineBasicMaterial.fromMap({'color': 0x323b4d}),
      ),
    );
    combat.onEvent = (actor, event) {
      unawaited(_combatEvent(actor, event));
    };
  ...
  Future<RenderPart> makePart(
    ...
    if (texture == null)
      throw FormatException('El motor no pudo cargar $texturePath');
    texture.colorSpace = t.SRGBColorSpace;
    texture.wrapS = t.RepeatWrapping;
    texture.wrapT = t.RepeatWrapping;
    final geometry = t.BufferGeometry();
    final positions = t.Float32BufferAttribute.fromList(
  ...
  Future<ClipData?> firstCompatible(Actor actor, List<String> paths) async {
    final sorted = List<String>.from(paths)
      ..sort((a, b) {
        int rank(String x) => x.contains('normal')
            ? 0
            : x.contains('_br')
            ? 1
            : x.contains('idle')
  ...
  Future<void> setAppearance(Appearance next) async {
    ...
    try {
      ...
      final c = await firstCompatible(staged, next.archetype.animations);
      if (c != null) {
        staged.idle = c;
        staged.normal = c;
        staged.play(c);
        ...
        if (mount != null) {
          final ride = await firstCompatible(
            staged,
            next.archetype.animations
                .where((p) => motionIndex(p) == 21)
                .toList(),
          );
          if (ride != null) staged.play(ride);
        }
      }
      if (disposed || revision != _appearanceRevision) {
        staged.dispose();
      ...
      updateCamera();
      say(
        'Apariencia aplicada · ${staged.parts.fold(0, (n, p) => n + p.data.triangles)} triángulos${c == null ? ' · sin animación compatible' : ''}.',
      );
    } catch (e) {
      staged.dispose();
  ...
  Future<void> selectCreature(CreatureRecord? c, String kind) async {
    ...
    if (kind == 'enemy') {
      ...
      combat.reset();
      if (staged != null) {
        riderHeight = (staged.height * .58).clamp(.2, 5.0);
        await riderPose();
  ...
  Future<void> riderPose() async {
    final a = character;
    if (a == null) return;
    final choices = animations
        .where(
          (p) => p.contains('veh') || p.contains('ride') || p.contains('sit'),
        )
        .toList();
    final c = await firstCompatible(a, choices);
    if (c != null && a == character) a.play(c);
  }
  ...
  Future<void> equip(WeaponRecord? w) async {
    ...
    final r = Bin(await lib.read(m), m);
    r.str();
    final data = MeshData.rigid(r);
    r.end();
    ...
    if ([5, 15].contains(weaponFamily(w)) && w.transforms[code][1].defined) {
      ...
      try {
        other = await makePart(data, tex, opaque: w.alpha == 1);
      } catch (_) {
        part.dispose();
        rethrow;
      }
  ...
  Future<void> prepareWeaponMotions() async {
    ...
    final candidates = wanted.isEmpty
        ? animations.where((p) => p.contains('attack')).take(4)
        : animations.where((p) => wanted.contains(motionIndex(p)));
    for (final path in candidates) {
      final c = await firstCompatible(a, [path]);
      if (c != null && a == character) attackClips.add(c);
    }
    if (a != character) return;
    final ready = readyMotion(family),
        paths = animations.where((p) => motionIndex(p) == ready).toList();
    final idle = paths.isEmpty ? a.normal : await firstCompatible(a, paths);
    if (idle != null) {
      a.idle = idle;
      if (mount == null) a.play(idle);
    }
  ...
  Future<void> locomotion(bool moving, bool fast) async {
    final revision = ++_locomotionRevision, a = character;
    if (a == null) return;
    ClipData? desired;
    final index = mount != null ? (moving ? 20 : 21) : (fast ? 2 : 1);
    if (!moving && mount == null) {
      desired = a.idle;
    } else {
      desired = await firstCompatible(
        a,
        animations.where((p) => motionIndex(p) == index).toList(),
      );
    }
    ...
    if (vehicle != null) {
      final c =
          vehicle.clips[moving ? (fast ? 'Correr' : 'Caminar') : 'Respirar'] ??
          vehicle.clips[moving ? 'Correr' : 'Reposo'];
  ...
  void updateAttachments() {
    ...
    if (wing != null) {
      ...
      final delta =
          ...
          : v.Matrix4.identity();
      final offset = v.Matrix4.compose(
        v.Vector3(0, wingHeight, wingDepth),
        v.Quaternion.identity(),
        v.Vector3.all(wingSize),
      );
      wing!.root.matrix.copyFromArray((root * delta * offset).storage);
  ...
  Future<void> playSound(String path) async {
    ...
    try {
      final bytes = await catalog!.library.read(path, limit: 32 * 1024 * 1024),
          dir = await getTemporaryDirectory();
      final safeName = baseName(
        path,
      ).replaceAll(RegExp(r'[^a-zA-Z0-9._-]'), '_');
      final f = File(
        '${dir.path}/shaiya_${path.hashCode.toUnsigned(32)}_$safeName',
      );
  ...
  Future<void> attack() async {
    ...
    character!.root.rotation.y = math.atan2(
  ...
  void resetCombat() {
    combat.reset();
    for (final a in [character, enemy]) {
      if (a == null) continue;
  ...
  void tick(double dt) {
    ...
    final delta = _frameAccumulator.clamp(0.0, .1);
    _frameAccumulator = 0;
    final moving = walkX != 0 || walkZ != 0;
    if (moving != _wasMoving || (moving && running != _wasRunning)) {
      _wasMoving = moving;
      _wasRunning = running;
      unawaited(locomotion(moving, running));
    }
    for (final a in [character, enemy, mount, wing]) {
      a?.tick(delta);
    }
    if (character != null && (walkX != 0 || walkZ != 0)) {
      final norm = math.max(1, math.sqrt(walkX * walkX + walkZ * walkZ));
      final x =
              character!.root.position.x +
              walkX / norm * delta * (running ? 4 : 2),
          z =
              character!.root.position.z +
              walkZ / norm * delta * (running ? 4 : 2);
      if (world == null || (x.abs() < 55 && z.abs() < 55)) {
        character!.root.position.x = x;
        character!.root.position.z = z;
        if (world != null)
          groundY = world!.heightAt(
            originX + x,
            originZ - z,
            scale: .02,
            offset: -200,
  ...
  Future<void> setWorld(String? path, {double? x, double? z}) async {
    ...
    if (path == null) {
      for (final p in environmentParts) {
        p.dispose();
      }
    ...
    final lib = catalog!.library,
        w = WorldData.parse(await lib.read(path), path);
    if (w.size == 0)
      throw const FormatException(
        'Este WLD es una mazmorra DG; su geometría aún no se interpreta. Selecciona un mapa exterior FLD.',
      );
    if (w.size < 128)
      throw const FormatException(
        'Este mapa es menor que el tamaño de sector configurado.',
      );
    final ox = (x ?? w.size / 2).clamp(64.0, w.size - 64.0),
        oz = (z ?? w.size / 2).clamp(64.0, w.size - 64.0);
    final stage = t.Group(), parts = <RenderPart>[];
    try {
      final grouped = <int, List<double>>{};
      final uv = <int, List<double>>{};
      final width = w.size ~/ 2 + 1;
      for (var dz = -64; dz < 64; dz += 2) {
        for (var dx = -64; dx < 64; dx += 2) {
          final xx = ox + dx, zz = oz + dz;
          final type = w.types[(zz ~/ 2) * width + xx ~/ 2];
          final layer = type < w.layers.length ? type : 0;
          final verts = grouped.putIfAbsent(layer, () => []),
              tex = uv.putIfAbsent(layer, () => []);
          final tiling = w.layers.isEmpty
              ? 4.0
              : math.max(.1, w.layers[layer].tile.abs());
          for (final point in [
            [0, 0],
            [2, 0],
            [0, 2],
            ...
            final px = xx + point[0], pz = zz + point[1];
            verts.addAll([
              px - ox,
              w.heightAt(px, pz, scale: .02, offset: -200),
              -(pz - oz),
            ]);
            tex.addAll([px / tiling, pz / tiling]);
          }
        }
      }
      for (final entry in grouped.entries) {
        if (w.layers.isEmpty) break;
        final layer = w.layers[entry.key];
        final tex = lib.resolve(layer.texture, [
          'terrain',
          'terrain/texture',
          'terrain/dds',
        ], uniqueFallback: true);
        if (tex == null) {
          report('Textura de terreno ausente: ${layer.texture}');
          continue;
        }
        ...
        final data = MeshData(
          ...
          no,
          Float32List.fromList(uv[entry.key]!),
        ...
        final part = await makePart(data, tex, opaque: true);
        parts.add(part);
        stage.add(part.mesh);
      }
      if (parts.isEmpty)
        throw const FormatException(
          'No se pudo construir el terreno de este sector.',
        );
      var loaded = 0;
      final nearby =
          w.objects
              .where(
                (o) =>
                    (o.position.x - ox).abs() < 78 &&
                    (o.position.z - oz).abs() < 78 &&
                    ['Building', 'Shape', 'Tree'].contains(o.category),
              )
              .toList()
            ..sort(
              (a, b) => ((a.position.x - ox).abs() + (a.position.z - oz).abs())
                  .compareTo(
                    (b.position.x - ox).abs() + (b.position.z - oz).abs(),
                  ),
            );
      for (final obj in nearby.take(80)) {
        final model = lib.resolve(obj.asset, ['entity/${obj.category}']);
        if (model == null || !model.endsWith('.smod')) continue;
        try {
          final objects = readSmod(await lib.read(model), model);
          final group = t.Group();
          for (final piece in objects) {
            final tex = lib.resolve(piece.texture, [
              'entity/${obj.category}',
              'entity/${obj.category}/texture',
              'entity/${obj.category}/textures',
            ], uniqueFallback: true);
            if (tex == null) continue;
            final p = await makePart(piece.mesh, tex);
            parts.add(p);
            group.add(p.mesh);
          }
          group.scale.z = -1;
      ...
      originX = ox;
      originZ = oz;
      groundY = w.heightAt(ox, oz, scale: .02, offset: -200);
      character?.root.position.setValues(0, groundY, 0);
      enemy?.root.position.setValues(1.8, groundY, 0);
      distance = 8;
      updateCamera();
      say(
        'Sector de 128 × 128 m · $loaded objetos · altura original. Sin colisión con edificios.',
      );
    } catch (_) {
      for (final p in parts) {
        p.dispose();
