...
class StudioScene extends ChangeNotifier {
  ...
  Future<RenderPart> makePartFromLibrary(
    ...
    final future = _textures.putIfAbsent(key, () async {
      // A raw mesh without a proven texture is rendered in a neutral material.
      // The UI states that no texture is associated; no filename guessing.
      final png = texturePath.isEmpty
          ? Uint8List.fromList([
              137,
              80,
              78,
          ...
          : await compute(_decodeTexture, {
              'bytes': await library.read(texturePath),
              'path': texturePath,
              'opaque': opaque,
            });
