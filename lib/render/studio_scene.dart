...
class StudioScene extends ChangeNotifier {
  ...
  int _appearanceRevision=0,_creatureRevision=0,_mountRevision=0,_wingRevision=0,_worldRevision=0,_weaponRevision=0,_clipRevision=0,_effectRevision=0;
  ...
  Future<void> setup(t.ThreeJS three) async {
    ...
    final points=<double>[];
    for(var i=-15;i<=15;i++){points.addAll([i.toDouble(),-.02,-15,i.toDouble(),-.02,15,-15,-.02,i.toDouble(),15,-.02,i.toDouble()]);}
    final gridGeometry=t.BufferGeometry()..setAttributeFromString('position',t.Float32BufferAttribute.fromList(points,3));
    three.scene.add(t.LineSegments(gridGeometry,t.LineBasicMaterial.fromMap({'color':0x323b4d})));
  ...
  Future<ClipData?> firstCompatible(Actor actor,List<String> paths) async {
    ...
    for(final path in sorted) {
      try{final c=actor.clips[path]??await clip(path);if(compatible(actor,c)){actor.clips[path]=c;return c;}}catch(e){report(e.toString());}
    }
    return null;
  }
  Future<void> setAppearance(Appearance next) async {
    ...
    try {
      ...
      final c=await firstCompatible(staged,next.archetype.animations);if(c!=null){staged.idle=c;staged.play(c);}
      if(disposed||revision!=_appearanceRevision){staged.dispose();return;}
      staged.root.scale.z=-1;weapon?.dispose();weapon=null;weaponRecord=null;weaponAttachment=null;++_weaponRevision;++_clipRevision;
      character?.dispose();character=staged;appearance=next;view!.scene.add(staged.root);
  ...
  Future<void> selectAnimation(String path) async {
    final a=character;if(a==null)return;final revision=++_clipRevision,c=a.clips[path]??await clip(path);
    ...
    a.clips[path]=c;a.play(c);say('${animationLabel(path)} · ${c.duration.toStringAsFixed(2)} s');
  }
  Future<Actor> loadCreature(CreatureRecord c) async {
    ...
    try {
      ...
      if(idle!=null){a.idle=idle;a.play(idle);}a.root.scale.z=-1;return a;
  ...
  Future<void> selectCreature(CreatureRecord? c,String kind) async {
    ...
    else if(kind=='mount'){mount?.dispose();mount=staged;mountRecord=c;combat.reset();if(staged!=null){riderHeight=(staged.height*.58).clamp(.2,5.0);await riderPose();}else if(character?.idle!=null){character!.play(character!.idle!);}}
  ...
  Future<void> equip(WeaponRecord? w) async {
    ...
    if(w==null){weapon?.dispose();weapon=null;weaponRecord=null;weaponAttachment=null;notifyListeners();return;}
  ...
  void updateAttachments() {
    final a=character;if(a==null)return;
    final x=a.root.position.x,z=a.root.position.z,rotation=a.root.rotation.y;
    ...
    if(wing!=null){wing!.root.position.setValues(x+math.sin(rotation)*wingDepth,groundY+wingHeight+(mount==null?0:riderHeight),z+math.cos(rotation)*wingDepth);wing!.root.scale.setValues(wingSize,wingSize,-wingSize);wing!.root.rotation.y=rotation;}
  }
  ...
  Future<void> setEffect(String path) async {
    final revision=++_effectRevision,lib=catalog!.library;final png=await compute(_decodeTexture,{'bytes':await lib.read(path),'path':path,'opaque':false});
    final texture=await t.TextureLoader(flipY:false).fromBytes(png);if(texture==null)return;
    if(disposed||revision!=_effectRevision){texture.dispose();return;}
  ...
  Future<void> attack() async {
    ...
    character!.root.rotation.y=math.atan2(enemy!.root.position.x-character!.root.position.x,enemy!.root.position.z-character!.root.position.z);
  ...
  void resetCombat(){combat.reset();for(final a in [character,enemy]){if(a==null)continue;final c=a.clips['Respirar']??a.clips['Reposo']??a.clips.values.where((c)=>c.source.contains('normal')||c.source.contains('_br')).firstOrNull;if(c!=null){a.idle=c;a.play(c);}}notifyListeners();}
  ...
  void tick(double dt) {
    ...
    if(character!=null&&(walkX!=0||walkZ!=0)){
      ...
      final x=character!.root.position.x+walkX/norm*delta*2,z=character!.root.position.z+walkZ/norm*delta*2;
  ...
  @override void dispose(){disposed=true;++_appearanceRevision;++_creatureRevision;++_mountRevision;++_wingRevision;++_worldRevision;++_weaponRevision;++_effectRevision;for(final a in [character,enemy,mount,wing]){a?.dispose();}weapon?.dispose();for(final p in environmentParts){p.dispose();}effectTexture?.dispose();audio.dispose();super.dispose();}
}
