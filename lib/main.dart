        ...
        return Column(
          ...
          children: [
            ...
            section('Cuerpo base', [
              ...
              note(
                'La base restaura torso, piernas, manos y pies del arquetipo, conservando tu rostro y cabello. Sus texturas se muestran sin modificarlas.',
              ),
            ]),
          ],
        );
          ...
          children: [
            section(
              ...
              [
                field<String>(
                  ...
                  [
                    originalBodySet,
                    if (a.hasNude) nudeBodySet,
                    ...a.sets.keys.where(
                      (key) => key != originalBodySet && key != nudeBodySet,
                    ),
                  ],
                  scene.appearance!.selectedSetKey ??
            ...
            section('Cuerpo / Nude', [
              ...
              OutlinedButton.icon(
                        ...
                        if (!a.hasNude) {
                          await showDialog<void>(
                            ...
                            builder: (ctx) => AlertDialog(
                              title: const Text('Recursos Nude del arquetipo'),
                              content: Text(
                                '${a.nudeUnavailableReason ?? "No hay un perfil Nude completo en DATA."}\n\nEl programa no reemplaza el cuerpo por una armadura teñida. Admite perfiles propios mediante shaiya-studio-bodies.json en DATA; el formato está documentado en docs/PERFILES_CUERPO.md.',
                              ),
                              actions: [
                                TextButton(
                ...
                icon: Icon(
                  a.hasNude ? Icons.person_outline : Icons.info_outline,
                  size: 16,
                ),
                label: Text(
                  a.hasNude
                      ? 'Aplicar Nude original'
                      : 'Nude · falta perfil original',
                  style: const TextStyle(fontSize: 11),
                ),
              ),
              note(
                a.hasNude
                    ? 'Se aplican la malla y la textura del perfil sin censura ni recoloreado; se conservan cara y cabello.'
                    : 'Nude no disponible para este arquetipo en la biblioteca actual. Todos los conjuntos originales siguen accesibles.',
              ),
            ]),
            section('Piezas', [
              for (final s in [
  ...
  Future<void> loadAppearance() async {
    ...
    for (final s in Slot.values) {
      ...
      slots[s] = id == null
          ...
          : [
              ...a.parts[s]!,
              ...a.nudeParts.values.where((p) => p.slot == s),
            ].where((p) => p.raw.id == id).firstOrNull;
    ...
    await scene.setAppearance(
      Appearance(
        ...
        selectedSetKey: data['setKey'] as String?,
      ),
    );
  }
