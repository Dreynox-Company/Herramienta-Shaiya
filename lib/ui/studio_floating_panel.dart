...
class _FloatingPanelState extends State<StudioFloatingPanel> {
  ...
  Rect get area {
    ...
    final width = math.min(
      maximized ? available.width - 16 : extent.width,
      available.width,
    );
    final height = math.min(
      maximized ? available.height - 16 : extent.height,
      available.height,
    );
    final initial =
        position ??
        Offset(widget.side == 'left' ? 54 : available.width - width - 16, 16);
    final origin = maximized ? const Offset(8, 8) : initial;
    return Rect.fromLTWH(
      origin.dx.clamp(0.0, math.max(0, available.width - width)).toDouble(),
  ...
  Widget build(BuildContext context) {
    ...
    return Positioned.fromRect(
      rect: rect,
      child: Material(
        key: ValueKey('floating-${widget.side}'),
        elevation: 14,
              ...
              children: [
                GestureDetector(
                  ...
                  onPanUpdate: maximized
                      ? null
                      : (e) =>
                            setState(() => position = rect.topLeft + e.delta),
                  child: MouseRegion(
                    cursor: maximized
                    ...
                    child: SizedBox(
                      ...
                      child: Row(
                        children: [
                          ...
                          IconButton(
                            key: ValueKey('maximize-${widget.side}'),
                            tooltip: maximized
                                ? 'Restaurar panel'
                                : 'Expandir panel flotante',
                            onPressed: () =>
                          ...
                          IconButton(
                            key: ValueKey('dock-${widget.side}'),
                            tooltip: 'Acoplar panel',
                            onPressed: widget.onDock,
                            icon: const Icon(Icons.vertical_split, size: 16),
                          ),
                          IconButton(
                            key: ValueKey('close-floating-${widget.side}'),
              ...
              Positioned(
                ...
                child: MouseRegion(
                  ...
                  child: GestureDetector(
                    key: ValueKey('floating-resize-${widget.side}'),
                    behavior: HitTestBehavior.opaque,
                    onPanUpdate: (e) => setState(() {
                      position = rect.topLeft;
                      extent = Size(
                        (rect.width + e.delta.dx)
                            .clamp(
                              240.0,
                              math.max(240, widget.bounds.width - rect.left),
                            )
                            .toDouble(),
                        (rect.height + e.delta.dy)
                            .clamp(
                              160.0,
                              math.max(160, widget.bounds.height - rect.top),
                            )
                            .toDouble(),
                      );
                    }),
