"""Extract the pinned public reference SAH/SAF for direct-DATA parity QA."""
...
with (base/'data.saf').open('rb') as source:
 for parts,offset,length in entries:
  ...
  total+=length
pathlib_info={'files':len(entries),'bytes':total,'mode':'native-reference raw DATA'}
print(json.dumps(pathlib_info))
