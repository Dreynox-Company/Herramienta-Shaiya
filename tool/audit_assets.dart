...
void main(List<String> args) async {
  if (args.isEmpty) {
    stderr.writeln('Uso: audit_assets_linux RUTA_DATA [informe.json]');
    exitCode = 64;
    return;
  }
  final root = Directory(args.first);
  if (!await root.exists()) {
    stderr.writeln('No existe DATA.');
    exitCode = 66;
    return;
  }
  final verified = <String, int>{}, errors = <Map<String, String>>[];
  var files = 0;
  await for (final e in root.list(recursive: true, followLinks: false)) {
    ...
    try {
      ...
      switch (ext) {
        case '3dc':
          MeshData.skinned(b, path);
          break;
        case 'ani':
          ClipData.parse(b, path);
          break;
        case 'mlt':
          readMlt(b, path);
          break;
        ...
        case 'mon':
          ...
          break;
        case 'dds':
          Pixels.decode(b, path);
          break;
        case 'wld':
          WorldData.parse(b, path);
          break;
      }
      verified.update(ext, (n) => n + 1, ifAbsent: () => 1);
    } catch (error) {
      errors.add({'file': path, 'error': error.toString()});
    }
    files++;
  ...
  stderr.writeln(
    'Completado: $files archivos, ${errors.length} incompatibilidades.',
  );
}
