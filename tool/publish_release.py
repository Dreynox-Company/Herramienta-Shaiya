...
def validate_environment(env: dict) -> tuple[str, int, str]:
    ...
    return sha, int(env['GITHUB_RUN_ID']), f'studio-0.6.0-{sha[:12]}'
...
def main() -> None:
    ...
    def api(method: str, url: str, data: bytes, mime: str = 'application/json') -> dict:
        req = Request(url, method=method, data=data, headers={
            'Authorization': f'Bearer {token}', 'User-Agent': 'Dreynox-Shaiya-Release/0.6.0',
    ...
    release = api('POST', f'{API}/releases', json.dumps({
        ...
        'name': f'Shaiya Studio 0.6.0 · {sha[:12]}', 'draft': True, 'prerelease': True, 'make_latest': 'false',
