...
void main() {
  ...
  testWidgets(
    'independent Flutter game: native rendering, play and save reload',
    (t) async {
      ...
      final saves = await Directory.systemTemp.createTemp(
        'flutter-game-native-',
      );
      ...
      try {
        ...
        await File('${out.path}/result.json').writeAsString(
          jsonEncode({
            'native_render': true,
            'scope': 'independent Flutter playtest; own synthetic resource fixtures, not the original MMO',
            'checks': checks,
          }),
        );
      } finally {
