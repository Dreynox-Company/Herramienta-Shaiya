...
void main() {
  ...
  testWidgets('native Windows SPK AES-GCM + Zstandard read and extraction', (
    tester,
  ) async {
    final temp = await Directory.systemTemp.createTemp('spk-native-test-');
    try {
      final original = Uint8List.fromList('recurso SPK nativo'.codeUnits);
      final spk = await _buildSyntheticSpk(original);
      final path = '${temp.path}${Platform.pathSeparator}data.spk';
      ...
      expect(source.validation['dataCoverageContiguous'], true);
