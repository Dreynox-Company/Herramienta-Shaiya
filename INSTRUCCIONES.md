# Instalar, compilar y publicar · revisión 0.3.1

## 0. Obtener los dos binarios con un único iniciador

Con **Git y Python 3** disponibles, abre `OBTENER_EXE_Y_APK.cmd`. No necesita Flutter, Visual Studio ni Android SDK en tu equipo: usa los runners de GitHub para compilar.

Solicita escribir **PUBLICAR**. Esa confirmación autoriza enviar solo las fuentes verificadas a una rama nueva, consumir minutos de Actions y publicar una versión preliminar pública de Windows/Android. Git utiliza tu sesión habitual; si pide iniciar sesión, completa su flujo normal. El programa no busca tokens ni cambia configuraciones globales.

Después de publicar, espera la ejecución exacta del commit, exige éxito y comprueba SHA-256 de ambos resultados antes de guardarlos en `dist/<commit>/`. Si falla, no descarga la versión 0.2 ni declara que los binarios existan. Guarda la solicitud y muestra un comando `--resume` para continuar sin publicar otra rama. El clon aislado se conserva en LocalAppData/Dreynox/ShaiyaStudio/compilaciones para que puedas revisarlo.

La identidad Git debe estar configurada. Si el repositorio bloquea los workflows o la publicación de versiones, el proceso se detiene con el enlace y el error; no intenta cambiar sus permisos. **La compilación remota de esta entrega aún no se ha ejecutado desde este chat.**

Alternativa con los SDK ya instalados: abre `GENERAR_LOCAL_WINDOWS_Y_ANDROID.cmd`, que ejecuta las dos compilaciones y abre el programa de Windows después del éxito. A continuación se detallan ambas opciones.
...
```text
C:\Proyectos\Shaiya-Studio-0.3.1\        Fuentes de esta entrega
...
dist\Shaiya_Studio_0_3_1_Windows.zip
```
...
```text
dist\Shaiya_Studio_0_3_1_Android_PRUEBAS.apk
```
...
Crea una rama nueva `feat/studio-031-integration`, añade exclusivamente las fuentes del manifiesto, crea el commit y lo envía sin force-push. No fusiona con `main`, no borra ramas y no hace stash de tus cambios. Las credenciales las gestiona Git; no se buscan ni se copian tokens.
...
## 6. Alcance y pruebas de esta revisión

La corrección actual tiene 99 pruebas Dart/Flutter, 22 pruebas Python y 60 comprobaciones de renderizado/integridad de los 20 cuerpos base originales mediante EGL/GLES3. No son una compilación ni una prueba física de Windows o Android de 0.3.1.

`Conjuntos → Cuerpo base original` restaura el cuerpo completo sin cambiar la cara y el cabello seleccionados. Se muestran las texturas originales, que contienen vestimenta. No se ha identificado un juego completo de mallas/texturas Nude originales para estos 20 arquetipos; no se oculta esa diferencia ni se generan texturas desnudas falsas.

Los controles de lapisia/elementos siguen siendo una previsualización de recetas originales, no una reproducción homologada de cada nivel del cliente. El agua avanzada y el intérprete completo de EFT continúan fuera de esta revisión.
